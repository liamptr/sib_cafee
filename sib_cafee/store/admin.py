...
class FoodAdmin(admin.ModelAdmin):
    ...
    def collection_title(self, food):
        return food.collection.title
...
class CollectionAdmin(admin.ModelAdmin):
    autocomplete_fields = ['featured_food']
    list_display = ['title', 'foods_count']
    search_fields = ['title']

    @admin.display(ordering='foods_count')
    def foods_count(self, collection):
        url = (
            reverse('admin:store_food_changelist')
            + '?'
            + urlencode({
                'collection__id': str(collection.id)
            }))
        return format_html('<a href="{}">{} Foods</a>', url, collection.foods_count)
